...
class YouTubePlaylistAdmin(admin.ModelAdmin):
    list_display = (
        'title', 'show_playlist_url', 'show_thumbnail', 'description',
        'channel',
    ...
    def show_video_of_playlist(self, obj):
        ...
        extra = "?channel__playlist__exact=%s" % (obj.title)
        return "<a href='%s' target='_blank'>查看视频</a>" % \
               (change_url + extra)

    show_video_of_playlist.allow_tags = True
    show_video_of_playlist.short_description = '查看视频'
    ...
    def get_playlist_video_info_url(self, obj):
        ...
        return "<a href='%s' target='_blank'>获取视频信息</a>" % get_playlist_video_url

    get_playlist_video_info_url.allow_tags = True
    get_playlist_video_info_url.short_description = '获取视频信息'
