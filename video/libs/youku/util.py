...
def check_error(response, expect_status1=200, expect_status2=201):
    """
    ...
    try:
        ...
        pass
    if ((response.status_code != expect_status1 and response.status_code !=
        expect_status2) or response.status_code == 400 or
                'error' in json):
        if json and 'error' in json:
            error = json['error']
            raise YoukuError(error['code'], error['type'],
                             error['description'], response.status_code)
        else:  # try to parse error from body
