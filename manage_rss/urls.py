...
urlpatterns = [
    ...
    url(r'^(?P<group_slug>[-\w]+)/rss/$', unpub_article_feed(), name='group_rss_url'),
    ...
    url(r'^set_publishable_status/(?P<article_id>\d+)/(?P<publishable_status>\w+)/$', set_publishable_status_view,
        name='set_publishable_status_url'),

    #设置文章的editable_status
    #例如 http://127.0.0.1:8000/manage_rss/set_editable_status/5/1/
    url(r'^set_editable_status/(?P<article_id>\d+)/(?P<editable_status>\w+)/$', set_editable_status_view,
