...
class Group(models.Model):
    ...
    def grab_article(self):
        html = "<a href='%s' target='_blank'>Grab</a>" % reverse(
            "grab_article_of_group", args=[self.pk])
        return html
    ...
    def rss_url(self):
        html = "<a href='%s' target='_blank'>Rss Url</a>" % reverse(
            "group_rss_url", args=[self.slug])
        return html
...
class Article(models.Model):
    ...
    read_status = models.BooleanField(default=False)  # TRUE表明已经生成RSS，FALSE
    # 表明未生成RSS
    editable_status = models.BooleanField(
        default=False)  # TRUE表明需要修改，FALSE表明不需要
    publishable_status = models.BooleanField(
        default=False)  # TRUE表明可以发布，FALSE表明不能发布
    ...
    def pub_article(self):
        if self.pub_info:
            ...
            html = "<a href='%s' target='_blank'>Pub</a>" % reverse(
                "pub_article_url",
    ...
    # https://docs.djangoproject.com/en/dev/ref/urlresolvers/#django.core
    # .urlresolvers.reverse
    def get_absolute_url(self):
        return reverse('article_url', kwargs={'article_id': str(self.id)})

    # https://docs.djangoproject.com/en/dev/ref/unicode/#choosing-between-str
    # -and-unicode
