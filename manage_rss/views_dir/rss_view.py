...
class unpub_article_feed(Feed):
    ...
    def link(self, obj):
        ...
        group_change_url = urlresolvers.reverse('admin:manage_rss_group_change',
                                                args=(obj.id,))
    ...
    def items(self, obj):
        # https://docs.djangoproject.com/en/dev/topics/db/queries/#backwards
        # -related-objects
        ...
        for rss in rss_set:
            ...
            article_set = Article.objects.filter(rss__exact=rss).filter(
                read_status__exact=True).order_by(
                '-grab_date')[:10]

            Article.objects.filter(pk__in=article_set).update(read_status=True)
            all_article_set = all_article_set + list(article_set)

        # 用 article_set.update(read_status=True) 会提示Cannot update a query
        # once a slice has been taken
        # 因为对queryset进行了slice之后无法再用update。参考
        # http://stackoverflow.com/a/4286037/1314124
    ...
    def item_description(self, item):
        ...
        # https://docs.djangoproject.com/en/dev/ref/contrib/sites/#getting
        # -the-current-domain-for-full-urls
        # 使用django自带的Site模块获取网站的网址
        set_publishable_status_url = Site.objects.get_current().domain + \
                                     reverse(
                                         'set_publishable_status_url',
                                         kwargs={'article_id': int(item.id),
                                                 'publishable_status': '1'})

        set_editable_status_url = Site.objects.get_current().domain + reverse(
            'set_editable_status_url',
            kwargs={'article_id': int(item.id),
                    'editable_status': '1'})

        article_url = Site.objects.get_current().domain + reverse('article_url',
                                                                  kwargs={
                                                                      'article_id': int(
                                                                          item.id)})

        # 在正文的后面加入可以发布文章的链接
        context = item.context + "<p><a href=' %s " \
                                 "'>set_publishable_status</a>  <p><a href=' " \
                                 "%s '>set_editable_status</a> " \
                                 "<p><a href=' %s '>article_url</a>" % (
                                     set_publishable_status_url,
                                     set_editable_status_url, article_url)
        return context
